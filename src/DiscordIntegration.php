...
{
    protected $apiKey = "fb164eb729c73fd5456f005f93a2715c";
    protected $user;
    ...
    public function listForms()
    {
        ...
        $formData = json_decode(
            Requests::getRequest(
                "https://api.jotform.com/user/forms?".'filter=%7B"status"%3A"ENABLED"%7D',
        ...
        foreach ($answers as $key) {
            if (!in_array($key['type'], $skippedFields) && in_array($key['order'], $questions)) {
                if (isset($key['prettyFormat'])) {
                    ...
                    } elseif ($key['type']=='control_datetime') {
                        $dateFormat = array();
                    ...
                    } elseif ($key['type']=='control_inline') {
                        $str = "";
                        $i =1 ;
                    ...
                    } elseif ($key['type']=='control_fileupload') {
                        //bury links
                    ...
                    } elseif ($key['type']=='control_matrix') {
                        $str = "";
                        foreach ($key['answer'] as $k=>$v) {
                            $str .= $k.' : '.$v.'\n';
                        }
                    ...
                    } elseif ($key['type']=='control_payment') {
                    ...
                    } elseif ($key["type"]=="control_signature") {
                        $val_arr[] =  '{
                                "name": "'.$key['text'].'",
